...
export const calcAge = (dob: any) => {
	...
	if (age.asMilliseconds() < 0) throw "Age is in the future!";
...
export const pluralize = (str: string, n: any) =>
...
export const findUniqueAddresses = (dataArray: any[]): string[] => {
...
export const findAddressOccurrences = (
	transactions: any,
...
export const generateWeekArray = (weeksAgo: any) => {
	let result = [];
...
export const updateChartArrayByWeek = (chartArray: any, dataArray: any) => {
...
export const fetchPricesForChunk = async (tokensChunk: any, chain: any) => {
	try {
		console.log(
			"Sending request with data:",
			JSON.stringify({
				tokens: tokensChunk.map((token: any) => ({
		...
		let addresses: any[] = [];
		tokensChunk.forEach(function (item: any) {
			addresses.push({
		...
		return pricesData.filter((price: any) => price !== null);
	} catch (error) {
...
export const fetchHistoricalPrices = async (array: any, chain: any) => {
	try {
		const response = await fetch(
			`${baseURL}/erc20/prices?include=percent_change&chain=${chain}`,
			{
...
export async function enrichTransfersWithPrices(
	...
	const uniqueTokenAddresses = Array.from(
		new Set(erc20_transfers.map((t: any) => t.address))
	...
	for (const chunk of tokenChunks) {
		...
		pricesChunk.forEach((priceInfo: any) => {
...
export async function getNativePrice(chain: any) {
	if (
...
export const fetchSinglePrice = async (tokenAddress: any, chain: any) => {
	try {
		const response = await fetch(
...
export async function enrichNFTTransactionsWithUSDValue(
	...
	const nftTransactions = transactions.filter(
		(tx: any) => tx.category === "NFT Sale" || tx.category === "NFT Purchase"
	);

	// Step 2: Prepare data for fetching prices.
	const priceQueries = nftTransactions.map((tx: any) => {
		const nativeToken = networkData.find(
			(net: any) => net.id === tx.chain
		).wrappedTokenAddress;
	...
	nftTransactions.forEach((tx: any) => {
		const priceData = historicalPrices.find(
			(price) =>
				...
				price.token_address ===
					networkData.find((net: any) => net.id === tx.chain)
						.wrappedTokenAddress
		);
	...
	nftTransactions.forEach((tx: any) => {
		if (tx.historical_price) {
			...
			tx.value_usd =
				parseFloat(ethers.formatUnits(tx.value, 18)) * tx.historical_price;
		} else {
			tx.value_usd = null;
		}
	});
...
export function formatPrice(price: any, precision = 2) {
...
export function formatNumber(numberOrString: any) {
...
export function formatPriceNumber(num: any) {
...
export function extractMetadataInfo(metadata: any) {
...
export function findEarliestAndLatestTimestamps(data: any) {
...
export function uniqueAddressesFromTransaction(transaction: any) {
	...
	if (transaction.token_transfers) {
		transaction.token_transfers.forEach((transfer: any) => {
	...
	if (transaction.nft_transfers) {
		transaction.nft_transfers.forEach((nft: any) => {
			if (nft.from_address) {
	...
	if (transaction.approvals) {
		transaction.approvals.forEach((approval: any) => {
...
export function serializeBigintsInObject(obj: any) {
	return JSON.parse(
